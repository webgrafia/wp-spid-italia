<?php

/**
 * SAML 2.0 remote IdP metadata for SimpleSAMLphp.
 *
 * Remember to remove the IdPs you don't use from this file.
 *
 * See: https://simplesamlphp.org/docs/stable/simplesamlphp-reference-idp-remote
 */
/*
 * Guest IdP. allows users to sign up and register. Great for testing!
 */
/* $metadata['https://openidp.feide.no'] = array(
  'name' => array(
  'en' => 'Feide OpenIdP - guest users',
  'no' => 'Feide Gjestebrukere',
  ),
  'description'          => 'Here you can login with your account on Feide RnD OpenID. If you do not already have an account on this identity provider, you can create a new one by following the create new account link and follow the instructions.',

  'SingleSignOnService'  => 'https://openidp.feide.no/simplesaml/saml2/idp/SSOService.php',
  'SingleLogoutService'  => 'https://openidp.feide.no/simplesaml/saml2/idp/SingleLogoutService.php',
  'certFingerprint'      => 'c9ed4dfb07caf13fc21e0fec1572047eb8a7a4cb'
  ); */




$metadata['https://idp.spid.gov.it'] = array (
  'entityid' => 'https://la-scuola.it/wp-content/plugins/wp-spid-italia/lib/www/module.php/saml/sp/metadata.php/default-sp',
  'description' =>
  array (
    'en' => 'IDP spid.gov.it',
    'it' => 'IDP spid.gov.it',
  ),
  'icon' => 'spid-idp-dummy.svg', 
  'OrganizationName' =>
  array (
    'en' => 'IDP spid.gov.it',
    'it' => 'IDP spid.gov.it',
  ),
  'name' =>
  array (
    'en' => 'IDP spid.gov.it',
    'it' => 'IDP spid.gov.it',
  ),
  'OrganizationDisplayName' =>
  array (
    'en' => 'IDP spid.gov.it',
    'it' => 'IDP spid.gov.it',
  ),
  'url' =>
  array (
    'en' => 'https://idp.spid.gov.it',
    'it' => 'https://idp.spid.gov.it',
  ),
  'OrganizationURL' =>
  array (
    'en' => 'https://idp.spid.gov.it',
    'it' => 'https://idp.spid.gov.it',
  ),
  'contacts' =>
  array (
  ),
  'metadata-set' => 'saml20-idp-remote',
  'sign.authnrequest' => true,
  'SingleSignOnService' =>
  array (
    
    0 =>
    array (
      'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
      'Location' => 'https://idp.spid.gov.it/samlsso',
    ),
    
    /*
    0 =>
    array (
      'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
      'Location' => 'https://spid-idp.linfabox.it/samlsso',
    ),
    */
  ),
  'SingleLogoutService' =>
  array (
    0 =>
    array (
      'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
      'Location' => 'https://idp.spid.gov.it/samlsso',
    ),
    1 =>
    array (
      'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
      'Location' => 'https://idp.spid.gov.it/samlsso',
    ),
  ),
  'ArtifactResolutionService' =>
  array (
  ),
  'NameIDFormats' =>
  array (
    0 => 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
  ),
  'keys' =>
  array (
    0 =>
    array (
      'encryption' => false,
      'signing' => true,
      'type' => 'X509Certificate',
      'X509Certificate' => 'MIICNTCCAZ6gAwIBAgIES343gjANBgkqhkiG9w0BAQUFADBVMQswCQYDVQQGEwJVUzELMAkGA1UE CAwCQ0ExFjAUBgNVBAcMDU1vdW50YWluIFZpZXcxDTALBgNVBAoMBFdTTzIxEjAQBgNVBAMMCWxv Y2FsaG9zdDAeFw0xMDAyMTkwNzAyMjZaFw0zNTAyMTMwNzAyMjZaMFUxCzAJBgNVBAYTAlVTMQsw CQYDVQQIDAJDQTEWMBQGA1UEBwwNTW91bnRhaW4gVmlldzENMAsGA1UECgwEV1NPMjESMBAGA1UE AwwJbG9jYWxob3N0MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCUp/oV1vWc8/TkQSiAvTou sMzOM4asB2iltr2QKozni5aVFu818MpOLZIr8LMnTzWllJvvaA5RAAdpbECb+48FjbBe0hseUdN5 HpwvnH/DW8ZccGvk53I6Orq7hLCv1ZHtuOCokghz/ATrhyPq+QktMfXnRS4HrKGJTzxaCcU7OQID AQABoxIwEDAOBgNVHQ8BAf8EBAMCBPAwDQYJKoZIhvcNAQEFBQADgYEAW5wPR7cr1LAdq+IrR44i QlRG5ITCZXY9hI0PygLP2rHANh+PYfTmxbuOnykNGyhM6FjFLbW2uZHQTY1jMrPprjOrmyK5sjJR O4d1DeGHT/YnIjs9JogRKv4XHECwLtIVdAbIdWHEtVZJyMSktcyysFcvuhPQK8Qc/E/Wq8uHSCo='
    ),
  ),
);

$metadata['https://validator.spid.gov.it'] = array (
  'entityid' => 'https://la-scuola.it/wp-content/plugins/wp-spid-italia/lib/www/module.php/saml/sp/metadata.php/default-sp',
  'description' =>
  array (
    'en' => 'AgID Validator',
    'it' => 'AgID Validator',
  ),
  'icon' => 'spid-idp-dummy.svg', 
  'OrganizationName' =>
  array (
    'en' => 'AgID Validator',
    'it' => 'AgID Validator',
  ),
  'name' =>
  array (
    'en' => 'AgID Validator',
    'it' => 'AgID Validator',
  ),
  'OrganizationDisplayName' =>
  array (
    'en' => 'AgID Validator',
    'it' => 'AgID Validator',
  ),
  'url' =>
  array (
    'en' => 'https://validator.spid.gov.it',
    'it' => 'https://validator.spid.gov.it',
  ),
  'OrganizationURL' =>
  array (
    'en' => 'https://validator.spid.gov.it',
    'it' => 'https://validator.spid.gov.it',
  ),
  'contacts' =>
  array (
  ),
  'metadata-set' => 'saml20-idp-remote',
  'sign.authnrequest' => true,
  'SingleSignOnService' =>
  array (
    
    0 =>
    array (
      'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
      'Location' => 'https://validator.spid.gov.it/samlsso',
    ),
    
    /*
    0 =>
    array (
      'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
      'Location' => 'https://validator.spid.gov.it/samlsso',
    ),
    */
  ),
  'SingleLogoutService' =>
  array (
    0 =>
    array (
      'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
      'Location' => 'https://validator.spid.gov.it/samlsso',
    ),
    1 =>
    array (
      'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
      'Location' => 'https://validator.spid.gov.it/samlsso',
    ),
  ),
  'ArtifactResolutionService' =>
  array (
  ),
  'NameIDFormats' =>
  array (
    0 => 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
  ),
  'keys' =>
  array (
    0 =>
    array (
      'encryption' => false,
      'signing' => true,
      'type' => 'X509Certificate',
      'X509Certificate' => ''
    ),
  ),
);

$metadata['https://loginspid.aruba.it'] = array (

  'entityid' => 'https://la-scuola.it/wp-content/plugins/wp-spid-italia/lib/www/module.php/saml/sp/metadata.php/default-sp',
  'description' => array (
    'en' => 'ArubaPEC S.p.A.',
    'it' => 'ArubaPEC S.p.A.',
  ),  
  'icon' => 'spid-idp-dummy.svg', 
  'OrganizationName' => array (
    'en' => 'ArubaPEC S.p.A.',
    'it' => 'ArubaPEC S.p.A.',
  ),
  'name' => array (
    'en' => 'ArubaPEC S.p.A.',
    'it' => 'ArubaPEC S.p.A.',
  ),
  'OrganizationDisplayName' => array (
    'en' => 'ArubaPEC S.p.A.',
    'it' => 'ArubaPEC S.p.A.',
  ),
  'url' => array (
    'en' => 'https://www.pec.it/',
    'it' => 'https://www.pec.it/',
  ),
  'OrganizationURL' => array (
    'en' => 'https://www.pec.it/',
    'it' => 'https://www.pec.it/',
  ),
  'contacts' => array (
  ),
  'metadata-set' => 'saml20-idp-remote',
  'sign.authnrequest' => true,
  'SingleSignOnService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://loginspid.aruba.it/ServiceLoginWelcome',
  ),

  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://loginspid.aruba.it/ServiceLoginWelcome',
  )
),
  'SingleLogoutService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://loginspid.aruba.it/ServiceLogoutRequest',
  ),
  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://loginspid.aruba.it/ServiceLogoutRequest',
  )
),
  'ArtifactResolutionService' => array (
  ),
  'NameIDFormats' => array (
    0 => 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
  ),
  'keys' => array(
		0 => array (
   'encryption' => false,
   'signing' => true,
   'type' => 'X509Certificate',
   'X509Certificate' => ''
),

	)
);


$metadata['https://identity.infocert.it'] = array (

  'entityid' => 'https://la-scuola.it/wp-content/plugins/wp-spid-italia/lib/www/module.php/saml/sp/metadata.php/default-sp',
  'description' => array (
    'en' => 'InfoCert S.p.A.',
    'it' => 'InfoCert S.p.A.',
  ),  
  'icon' => 'spid-idp-dummy.svg', 
  'OrganizationName' => array (
    'en' => 'InfoCert S.p.A.',
    'it' => 'InfoCert S.p.A.',
  ),
  'name' => array (
    'en' => 'InfoCert S.p.A.',
    'it' => 'InfoCert S.p.A.',
  ),
  'OrganizationDisplayName' => array (
    'en' => 'InfoCert S.p.A.',
    'it' => 'InfoCert S.p.A.',
  ),
  'url' => array (
    'en' => 'https://www.infocert.it',
    'it' => 'https://www.infocert.it',
  ),
  'OrganizationURL' => array (
    'en' => 'https://www.infocert.it',
    'it' => 'https://www.infocert.it',
  ),
  'contacts' => array (
  ),
  'metadata-set' => 'saml20-idp-remote',
  'sign.authnrequest' => true,
  'SingleSignOnService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://identity.infocert.it/spid/samlsso',
  ),

  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://identity.infocert.it/spid/samlsso',
  )
),
  'SingleLogoutService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://identity.infocert.it/spid/samlslo',
  ),
  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://identity.infocert.it/spid/samlslo',
  )
),
  'ArtifactResolutionService' => array (
  ),
  'NameIDFormats' => array (
    0 => 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
  ),
  'keys' => array(
		0 => array (
   'encryption' => false,
   'signing' => true,
   'type' => 'X509Certificate',
   'X509Certificate' => ''
),

	)
);


$metadata['https://spid.intesa.it'] = array (

  'entityid' => 'https://la-scuola.it/wp-content/plugins/wp-spid-italia/lib/www/module.php/saml/sp/metadata.php/default-sp',
  'description' => array (
    'en' => 'IN.TE.S.A. S.p.A.',
    'it' => 'IN.TE.S.A. S.p.A.',
  ),  
  'icon' => 'spid-idp-dummy.svg', 
  'OrganizationName' => array (
    'en' => 'IN.TE.S.A. S.p.A.',
    'it' => 'IN.TE.S.A. S.p.A.',
  ),
  'name' => array (
    'en' => 'IN.TE.S.A. S.p.A.',
    'it' => 'IN.TE.S.A. S.p.A.',
  ),
  'OrganizationDisplayName' => array (
    'en' => 'Intesa S.p.A.',
    'it' => 'Intesa S.p.A.',
  ),
  'url' => array (
    'en' => 'https://www.intesa.it/',
    'it' => 'https://www.intesa.it/',
  ),
  'OrganizationURL' => array (
    'en' => 'https://www.intesa.it/',
    'it' => 'https://www.intesa.it/',
  ),
  'contacts' => array (
  ),
  'metadata-set' => 'saml20-idp-remote',
  'sign.authnrequest' => true,
  'SingleSignOnService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://spid.intesa.it/Time4UserServices/services/idp/AuthnRequest/',
  ),

  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://spid.intesa.it/Time4UserServices/services/idp/AuthnRequest/',
  )
),
  'SingleLogoutService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://spid.intesa.it/Time4UserServices/services/idp/SingleLogout',
  ),
  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://spid.intesa.it/Time4UserServices/services/idp/SingleLogout',
  )
),
  'ArtifactResolutionService' => array (
  ),
  'NameIDFormats' => array (
    0 => 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
  ),
  'keys' => array(
		0 => array (
   'encryption' => false,
   'signing' => true,
   'type' => 'X509Certificate',
   'X509Certificate' => ''
),

	)
);


$metadata['https://id.lepida.it/idp/shibboleth'] = array (

  'entityid' => 'https://la-scuola.it/wp-content/plugins/wp-spid-italia/lib/www/module.php/saml/sp/metadata.php/default-sp',
  'description' => array (
    'en' => 'Lepida S.p.A.',
    'it' => 'Lepida S.p.A.',
  ),  
  'icon' => 'spid-idp-dummy.svg', 
  'OrganizationName' => array (
    'en' => 'Lepida S.p.A.',
    'it' => 'Lepida S.p.A.',
  ),
  'name' => array (
    'en' => 'Lepida S.p.A.',
    'it' => 'Lepida S.p.A.',
  ),
  'OrganizationDisplayName' => array (
    'en' => 'Lepida S.p.A.',
    'it' => 'Lepida S.p.A.',
  ),
  'url' => array (
    'en' => 'https://www.lepida.it/',
    'it' => 'https://www.lepida.it/',
  ),
  'OrganizationURL' => array (
    'en' => 'https://www.lepida.it/',
    'it' => 'https://www.lepida.it/',
  ),
  'contacts' => array (
  ),
  'metadata-set' => 'saml20-idp-remote',
  'sign.authnrequest' => true,
  'SingleSignOnService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://id.lepida.it/idp/profile/SAML2/Redirect/SSO',
  ),

  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://id.lepida.it/idp/profile/SAML2/POST/SSO',
  )
),
  'SingleLogoutService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://id.lepida.it/idp/profile/SAML2/Redirect/SLO',
  ),
  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://id.lepida.it/idp/profile/SAML2/POST/SLO',
  )
),
  'ArtifactResolutionService' => array (
  ),
  'NameIDFormats' => array (
    0 => 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
  ),
  'keys' => array(
		0 => array (
   'encryption' => false,
   'signing' => true,
   'type' => 'X509Certificate',
   'X509Certificate' => ''
),

	)
);


$metadata['https://idp.namirialtsp.com/idp'] = array (

  'entityid' => 'https://la-scuola.it/wp-content/plugins/wp-spid-italia/lib/www/module.php/saml/sp/metadata.php/default-sp',
  'description' => array (
    'en' => 'Namirial',
    'it' => 'Namirial',
  ),  
  'icon' => 'spid-idp-dummy.svg', 
  'OrganizationName' => array (
    'en' => 'Namirial',
    'it' => 'Namirial',
  ),
  'name' => array (
    'en' => 'Namirial',
    'it' => 'Namirial',
  ),
  'OrganizationDisplayName' => array (
    'en' => 'Namirial S.p.a. Trust Service
                Provider',
    'it' => 'Namirial S.p.a. Trust Service
                Provider',
  ),
  'url' => array (
    'en' => 'https://www.namirialtsp.com',
    'it' => 'https://www.namirialtsp.com',
  ),
  'OrganizationURL' => array (
    'en' => 'https://www.namirialtsp.com',
    'it' => 'https://www.namirialtsp.com',
  ),
  'contacts' => array (
  ),
  'metadata-set' => 'saml20-idp-remote',
  'sign.authnrequest' => true,
  'SingleSignOnService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://idp.namirialtsp.com/idp/profile/SAML2/Redirect/SSO',
  ),

  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://idp.namirialtsp.com/idp/profile/SAML2/POST/SSO',
  )
),
  'SingleLogoutService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://idp.namirialtsp.com/idp/profile/SAML2/Redirect/SLO',
  ),
  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://idp.namirialtsp.com/idp/profile/SAML2/POST/SLO',
  )
),
  'ArtifactResolutionService' => array (
  ),
  'NameIDFormats' => array (
    0 => 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
  ),
  'keys' => array(
		0 => array (
   'encryption' => false,
   'signing' => true,
   'type' => 'X509Certificate',
   'X509Certificate' => ''
),

	)
);


$metadata['https://posteid.poste.it'] = array (

  'entityid' => 'https://la-scuola.it/wp-content/plugins/wp-spid-italia/lib/www/module.php/saml/sp/metadata.php/default-sp',
  'description' => array (
    'en' => 'Poste Italiane SpA',
    'it' => 'Poste Italiane SpA',
  ),  
  'icon' => 'spid-idp-dummy.svg', 
  'OrganizationName' => array (
    'en' => 'Poste Italiane SpA',
    'it' => 'Poste Italiane SpA',
  ),
  'name' => array (
    'en' => 'Poste Italiane SpA',
    'it' => 'Poste Italiane SpA',
  ),
  'OrganizationDisplayName' => array (
    'en' => 'Poste Italiane
                SpA',
    'it' => 'Poste Italiane
                SpA',
  ),
  'url' => array (
    'en' => 'https://www.poste.it',
    'it' => 'https://www.poste.it',
  ),
  'OrganizationURL' => array (
    'en' => 'https://www.poste.it',
    'it' => 'https://www.poste.it',
  ),
  'contacts' => array (
  ),
  'metadata-set' => 'saml20-idp-remote',
  'sign.authnrequest' => true,
  'SingleSignOnService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://posteid.poste.it/jod-fs/ssoserviceredirect',
  ),

  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://posteid.poste.it/jod-fs/ssoservicepost',
  )
),
  'SingleLogoutService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://posteid.poste.it/jod-fs/sloserviceredirect',
  ),
  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://posteid.poste.it/jod-fs/sloservicepost',
  )
),
  'ArtifactResolutionService' => array (
  ),
  'NameIDFormats' => array (
    0 => 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
  ),
  'keys' => array(
		0 => array (
   'encryption' => false,
   'signing' => true,
   'type' => 'X509Certificate',
   'X509Certificate' => ''
),

	)
);


$metadata['https://identity.sieltecloud.it'] = array (

  'entityid' => 'https://la-scuola.it/wp-content/plugins/wp-spid-italia/lib/www/module.php/saml/sp/metadata.php/default-sp',
  'description' => array (
    'en' => 'Sielte S.p.A.',
    'it' => 'Sielte S.p.A.',
  ),  
  'icon' => 'spid-idp-dummy.svg', 
  'OrganizationName' => array (
    'en' => 'Sielte S.p.A.',
    'it' => 'Sielte S.p.A.',
  ),
  'name' => array (
    'en' => 'Sielte S.p.A.',
    'it' => 'Sielte S.p.A.',
  ),
  'OrganizationDisplayName' => array (
    'en' => 'http://www.sielte.it',
    'it' => 'http://www.sielte.it',
  ),
  'url' => array (
    'en' => 'http://www.sielte.it',
    'it' => 'http://www.sielte.it',
  ),
  'OrganizationURL' => array (
    'en' => 'http://www.sielte.it',
    'it' => 'http://www.sielte.it',
  ),
  'contacts' => array (
  ),
  'metadata-set' => 'saml20-idp-remote',
  'sign.authnrequest' => true,
  'SingleSignOnService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://identity.sieltecloud.it/simplesaml/saml2/idp/SSO.php',
  ),

  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://identity.sieltecloud.it/simplesaml/saml2/idp/SSO.php',
  )
),
  'SingleLogoutService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://identity.sieltecloud.it/simplesaml/saml2/idp/SLS.php',
  ),
  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://identity.sieltecloud.it/simplesaml/saml2/idp/SLS.php',
  )
),
  'ArtifactResolutionService' => array (
  ),
  'NameIDFormats' => array (
    0 => 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
  ),
  'keys' => array(
		0 => array (
   'encryption' => false,
   'signing' => true,
   'type' => 'X509Certificate',
   'X509Certificate' => ''
),
		1 => array (
   'encryption' => false,
   'signing' => true,
   'type' => 'X509Certificate',
   'X509Certificate' => ''
),

	)
);


$metadata['https://spid.register.it'] = array (

  'entityid' => 'https://la-scuola.it/wp-content/plugins/wp-spid-italia/lib/www/module.php/saml/sp/metadata.php/default-sp',
  'description' => array (
    'en' => 'Register.it S.p.A.',
    'it' => 'Register.it S.p.A.',
  ),  
  'icon' => 'spid-idp-dummy.svg', 
  'OrganizationName' => array (
    'en' => 'Register.it S.p.A.',
    'it' => 'Register.it S.p.A.',
  ),
  'name' => array (
    'en' => 'Register.it S.p.A.',
    'it' => 'Register.it S.p.A.',
  ),
  'OrganizationDisplayName' => array (
    'en' => 'Register.it
                S.p.A.',
    'it' => 'Register.it
                S.p.A.',
  ),
  'url' => array (
    'en' => 'https//www.register.it',
    'it' => 'https//www.register.it',
  ),
  'OrganizationURL' => array (
    'en' => 'https//www.register.it',
    'it' => 'https//www.register.it',
  ),
  'contacts' => array (
  ),
  'metadata-set' => 'saml20-idp-remote',
  'sign.authnrequest' => true,
  'SingleSignOnService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://spid.register.it/login/sso',
  ),

  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://spid.register.it/login/sso',
  )
),
  'SingleLogoutService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://spid.register.it/login/singleLogout',
  ),
  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://spid.register.it/login/singleLogout',
  )
),
  'ArtifactResolutionService' => array (
  ),
  'NameIDFormats' => array (
    0 => 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
  ),
  'keys' => array(
		0 => array (
   'encryption' => false,
   'signing' => true,
   'type' => 'X509Certificate',
   'X509Certificate' => ''
),

	)
);


$metadata['https://login.id.tim.it/affwebservices/public/saml2sso'] = array (

  'entityid' => 'https://la-scuola.it/wp-content/plugins/wp-spid-italia/lib/www/module.php/saml/sp/metadata.php/default-sp',
  'description' => array (
    'en' => 'TI Trust Technologies srl',
    'it' => 'TI Trust Technologies srl',
  ),  
  'icon' => 'spid-idp-dummy.svg', 
  'OrganizationName' => array (
    'en' => 'TI Trust Technologies srl',
    'it' => 'TI Trust Technologies srl',
  ),
  'name' => array (
    'en' => 'TI Trust Technologies srl',
    'it' => 'TI Trust Technologies srl',
  ),
  'OrganizationDisplayName' => array (
    'en' => 'Trust Technologies
                srl',
    'it' => 'Trust Technologies
                srl',
  ),
  'url' => array (
    'en' => 'https://www.trusttechnologies.it',
    'it' => 'https://www.trusttechnologies.it',
  ),
  'OrganizationURL' => array (
    'en' => 'https://www.trusttechnologies.it',
    'it' => 'https://www.trusttechnologies.it',
  ),
  'contacts' => array (
  ),
  'metadata-set' => 'saml20-idp-remote',
  'sign.authnrequest' => true,
  'SingleSignOnService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://login.id.tim.it/affwebservices/public/saml2sso',
  ),

  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://login.id.tim.it/affwebservices/public/saml2sso',
  )
),
  'SingleLogoutService' => array (
  0 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
    'Location' => 'https://login.id.tim.it/affwebservices/public/saml2slo',
  ),
  1 => array (
    'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
    'Location' => 'https://login.id.tim.it/affwebservices/public/saml2slo',
  )
),
  'ArtifactResolutionService' => array (
  ),
  'NameIDFormats' => array (
    0 => 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
  ),
  'keys' => array(
		0 => array (
   'encryption' => false,
   'signing' => true,
   'type' => 'X509Certificate',
   'X509Certificate' => ''
),
		1 => array (
   'encryption' => false,
   'signing' => true,
   'type' => 'X509Certificate',
   'X509Certificate' => ''
),

	)
);



/*validator localhost*/


$metadata['http://la-scuola.it:8080'] = array (
  'entityid' => 'http://la-scuola.it:8080',
  'description' => 
  array (
    'it' => 'Validator2',
    'en' => 'Validator2',
    'fr' => 'Validator2',
    'de' => 'Validator2',
  ),
  'OrganizationName' => 
  array (
    'it' => 'Validator2',
    'en' => 'Validator2',
    'fr' => 'Validator2',
    'de' => 'Validator2',
  ),
  'name' => 
  array (
    'it' => 'Validator2',
    'en' => 'Validator2',
    'fr' => 'Validator2',
    'de' => 'Validator2',
  ),
  'OrganizationDisplayName' => 
  array (
    'it' => 'Validator2',
    'en' => 'Validator2',
    'fr' => 'Validator2',
    'de' => 'Validator2',
  ),
  'url' => 
  array (
    'it' => 'http://la-scuola.it:8080',
    'en' => 'http://la-scuola.it:8080',
    'fr' => 'http://la-scuola.it:8080',
    'de' => 'http://la-scuola.it:8080',
  ),
  'OrganizationURL' => 
  array (
      'it' => 'http://la-scuola.it:8080',
    'en' => 'http://la-scuola.it:8080',
    'fr' => 'http://la-scuola.it:8080',
    'de' => 'http://la-scuola.it:8080',
  ),
  'contacts' => 
  array (
  ),
  'metadata-set' => 'saml20-idp-remote',
  'sign.authnrequest' => true,
  'SingleSignOnService' => 
  array (
    0 => 
    array (
      'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
      'Location' => 'http://la-scuola.it:8080/samlsso',
    ),
    1 => 
    array (
      'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
      'Location' => 'http://la-scuola.it:8080/samlsso',
    ),
  ),
  'SingleLogoutService' => 
  array (
    0 => 
    array (
      'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
      'Location' => 'http://la-scuola.it:8080/samlsso',
      'ResponseLocation' => 'http://la-scuola.it:8080/samlslo',
    ),
    1 => 
    array (
      'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
      'Location' => 'http://la-scuola.it:8080/samlsso',
      'ResponseLocation' => 'http://la-scuola.it:8080/samlslo',
    ),
  ),
  'ArtifactResolutionService' => 
  array (
  ),
  'NameIDFormats' => 
  array (
    0 => 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
  ),
  'keys' => 
  array (
    0 => 
    array (
      'encryption' => false,
      'signing' => true,
      'type' => 'X509Certificate',
      'X509Certificate' => '',
    ),
  ),
);

/*FINE validator */
